...
def output(filename, sheet,num, name, present):
    my_file = Path('firebase/attendence_files/'+filename+str(datetime.now().date())+'.xls');
    if my_file.is_file():
        rb = open_workbook('firebase/attendence_files/'+filename+str(datetime.now().date())+'.xls');
        book = copy(rb);
        sh = book.get_sheet(0)
        # file exists
    else:
    ...
    book.save('firebase/attendence_files/'+fullname)
    return fullname;
